...
class ExpQuad_Identity_DirectionalDerivative(JaxKernel):
    ...
    def _evaluate(self, x0: np.ndarray, x1: Optional[np.ndarray]) -> np.ndarray:
        ...
        return self._expquad._output_scale_sq * proj_diffs * np.exp(-0.5 * dists_sq)
    ...
    def _evaluate_jax(self, x0: jnp.ndarray, x1: Optional[jnp.ndarray]) -> jnp.ndarray:
        ...
        return self._expquad._output_scale_sq * proj_diffs * jnp.exp(-0.5 * dists_sq)
...
class ExpQuad_DirectionalDerivative_DirectionalDerivative(JaxKernel):
    ...
    def _rescaled_direction0(self) -> np.ndarray:
        return self._direction0 / self._expquad._lengthscales ** 2

    @functools.cached_property
    def _rescaled_direction1(self) -> np.ndarray:
        return self._direction1 / self._expquad._lengthscales ** 2
    ...
    def _evaluate(self, x0: np.ndarray, x1: Optional[np.ndarray]) -> np.ndarray:
        if x1 is None:
            return np.full_like(  # pylint: disable=unexpected-keyword-arg
                x0,
                self._expquad._output_scale_sq * self._directions_inprod,
                shape=x0.shape[: x0.ndim - self.input_ndim],
            )
        ...
        dists_sq = self._batched_euclidean_norm_sq(diffs / self._expquad._lengthscales)
    ...
    def _evaluate_jax(self, x0: jnp.ndarray, x1: Optional[jnp.ndarray]) -> jnp.ndarray:
        ...
        dists_sq = self._batched_euclidean_norm_sq_jax(
            diffs / self._expquad._lengthscales
        )
