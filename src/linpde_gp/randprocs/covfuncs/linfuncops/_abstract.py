...
class CrossCovarianceFunction_Identity_LinearFunctionOperator(
    ...
    def __init__(
        self,
        covfunc: covfuncs.CovarianceFunction,
        ...
        if self._L.input_domain_shape != (
            self._covfunc.input_shape_0
            if self._reverse
            else self._covfunc.input_shape_1
        ):
            raise ValueError()
    ...
    @property
    def covfunc(self) -> covfuncs.CovarianceFunction:
...
class CovarianceFunction_LinearFunctionOperator(covfuncs.CovarianceFunction):
    def __init__(
        ...
        if self._L0.input_domain_shape != self._covfunc.input_shape_0:
            raise ValueError()

        if self._L0.input_codomain_shape != self._covfunc.output_shape_0:
            raise ValueError()

        if self._L1.input_domain_shape != self._covfunc.input_shape_1:
            raise ValueError()

        if self._L1.input_codomain_shape != self._covfunc.output_shape_1:
            raise ValueError()
